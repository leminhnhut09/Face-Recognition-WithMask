...
class FaceRecognizer:
    ...
    def create_known_faces(self, root):
        ...
        for i, folder in enumerate(os.listdir(root)):
                 # dir = os.listdir(os.path.join(root, folder))
            dir = os.path.join(root, folder)
            for  i, fname in enumerate(os.listdir(dir)):
                print(fname)
                fpath = os.path.join(dir, fname)

                item = self._get_img_face_encoding(fpath)
                if item:
                    self._known_faces.append(self._get_img_face_encoding(fpath))
                    self._know_name.append(folder)

        self._known_faces = np.asarray(self._known_faces)
        self._know_name = np.asarray(self._know_name)
        np.savez_compressed('data/embed_test', self._known_faces, self._know_name)
        print('So luong anh ', len(self._known_faces), len(self._know_name))
        print('tao thanh cong')
    def create_known_faces_read(self, known_faces, know_name):
        self._known_faces = known_faces
        self._know_name = know_name
