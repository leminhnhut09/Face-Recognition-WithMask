...
def parse_args():
    ...
    # general
    # Cấu hình mặc định của tập dữ liệu huấn luyện
    parser.add_argument('--dataset', default=default.dataset, help='dataset config')

    # Lựa chọn cấu trúc mạng mặc định
    parser.add_argument('--network', default=default.network, help='network config')

    # Sử dụng chức năng mất mặc định
    ...
    # Việc xác minh được thực hiện mỗi đợt dài dòng
...
def get_symbol(args):
    # Nhận một vector tính năng
    embedding = eval(config.net_name).get_symbol()

    # Xác định trình giữ chỗ thẻ để lưu trữ thẻ
    all_label = mx.symbol.Variable('softmax_label')
    gt_label = all_label
    is_softmax = True
    print('config.loss_name: ',config.loss_name)
    #print('config.numclasses: ',config.num_classes)
    print('config.loss_s: ',config.loss_s)
    # Nếu chức năng mất là softmax
    if config.loss_name == 'softmax':
        # Xác định trọng lượng của một lớp được kết nối đầy đủ
        ...
        # Nếu bạn không đặt thiên vị, thì liên kết trực tiếp
        if config.fc7_no_bias:
            fc7 = mx.sym.FullyConnected(data=embedding, weight=_weight, no_bias=True, num_hidden=config.num_classes,
                                        name='fc7')
        # Nếu _bias được đặt, nó sẽ được kết nối đầy đủ sau khi _bias được tạo
        else:
            _bias = mx.symbol.Variable('fc7_bias', lr_mult=2.0, wd_mult=0.0)
            fc7 = mx.sym.FullyConnected(data=embedding, weight=_weight, bias=_bias, num_hidden=config.num_classes,
                                        name='fc7')
    # Nếu hàm mất là margin_softmax
    elif config.loss_name == 'margin_softmax':
        # Tạo một giữ chỗ trọng lượng
        _weight = mx.symbol.Variable("fc7_weight", shape=(config.num_classes, config.emb_size),
                                     lr_mult=config.fc7_lr_mult, wd_mult=config.fc7_wd_mult, init=mx.init.Normal(0.01))


        # Lấy hệ số thu phóng của m bị mất
        s = config.loss_s

        # Chuẩn hóa L2 đầu tiên, sau đó liên kết đầy đủ
        ...
        # Sự tồn tại của nó m1, m2, m3 là tích hợp thuật toán lại với nhau
        if config.loss_m1 != 1.0 or config.loss_m2 != 0.0 or config.loss_m3 != 0.0:
            if config.loss_m1 == 1.0 and config.loss_m2 == 0.0:
                ...
                fc7 = fc7 + body
    # Nếu chức năng mất là bộ ba triplet
    ...
    # Nếu nó là một bộ ba triplet
    else:
        out_list.append(mx.sym.BlockGrad(gt_label))
        out_list.append(triplet_loss)

    # Tập hợp tất cả các biểu tượng
    out = mx.symbol.Group(out_list)
    return out


def train_net(args):
    # Xác định xem nên sử dụng GPU hay CPU
    ctx = []
    ...
    print('config.imageshape: ',config.image_shape[2])
    # Lưu tiền tố mô hình
    ...
    # Tổng số tính toán batch_size
    args.batch_size = args.per_batch_size * args.ctx_num
    args.rescale_threshold = 0

    args.image_channel = config.image_shape[2]
    config.batch_size = args.batch_size
    # Một kích thước lô cho mỗi GPU
    config.per_batch_size = args.per_batch_size

    # Danh mục dữ liệu đào tạo
    data_dir = config.dataset_path
    path_imgrec = None
    path_imglist = None

    # Kích thước hình ảnh và xác minh
    image_size = config.image_shape[0:2]
    assert len(image_size) == 2
    assert image_size[0] == image_size[1]
    print('image_size', image_size)

    # Số lượng id tập dữ liệu
    ...
    # Xác định xem mô hình được đào tạo trước có tồn tại hay không, nếu không, hãy khởi tạo các trọng số
    if len(args.pretrained) == 0:
        arg_params = None
        aux_params = None
        sym = get_symbol(args)  # Xây dựng mô hình
        if config.net_name == 'spherenet':
            data_shape_dict = {'data': (args.per_batch_size,) + data_shape}
            spherenet.init_weights(sym, data_shape_dict, args.num_layers)
    else:  # Nếu nó tồn tại, tải mô hình
        print('loading', args.pretrained, args.pretrained_epoch)
        _, arg_params, aux_params = mx.model.load_checkpoint(args.pretrained, args.pretrained_epoch)
        sym = get_symbol(args)

    # Tính toán không gian bị chiếm bởi dữ liệu dấu phẩy động
    ...
    # Trình lặp chủ yếu lấy dữ liệu, các trình lặp của dữ liệu đầu vào triplet và sfotmax là khác nhau và các chi tiết cụ thể khác nhau. Các chương tiếp theo sẽ phân tích cho bạn
    ...
    # Tải tất cả các bộ dữ liệu thử nghiệm
    ver_list = []
    ver_name_list = []
    ...
    def _batch_callback(param):
        ...
        # Giảm tỷ lệ học tập xuống một phần mười của bản gốc
        ...
        _cb(param)
        print('params: ',param.epoch,param.nbatch)
        # In mỗi 1000 đợt
        if mbatch % 1000 == 0:
    ...
    epoch_cb = None
    # Chuyển đổi train_dataiter sang mx.ioPrefetchingIter iterator
    train_dataiter = mx.io.PrefetchingIter(train_dataiter)
    #print('metric: ',eval_metrics)
    history=model.fit(train_dataiter,
